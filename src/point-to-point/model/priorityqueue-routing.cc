 ...
 */
PriorityQueueDataTag::PriorityQueueDataTag() : Tag() {}
TypeId PriorityQueueDataTag::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::PriorityQueueDataTag").SetParent<Tag>().AddConstructor<PriorityQueueDataTag>();
    return tid;
}
void PriorityQueueDataTag::SetPathId(uint32_t pathId) { m_pathId = pathId; }
uint32_t PriorityQueueDataTag::GetPathId(void) const { return m_pathId; }
void PriorityQueueDataTag::SetHopCount(uint32_t hopCount) { m_hopCount = hopCount; }
uint32_t PriorityQueueDataTag::GetHopCount(void) const { return m_hopCount; }
void PriorityQueueDataTag::SetEpoch(uint32_t epoch) { m_epoch = epoch; }
uint32_t PriorityQueueDataTag::GetEpoch(void) const { return m_epoch; }
void PriorityQueueDataTag::SetPhase(uint32_t phase) { m_phase = phase; }
uint32_t PriorityQueueDataTag::GetPhase(void) const { return m_phase; }
void PriorityQueueDataTag::SetTimestampTx(uint64_t timestamp) { m_timestampTx = timestamp; }
uint64_t PriorityQueueDataTag::GetTimestampTx(void) const { return m_timestampTx; }
void PriorityQueueDataTag::SetTimestampTail(uint64_t timestamp) { m_timestampTail = timestamp; }
uint64_t PriorityQueueDataTag::GetTimestampTail(void) const { return m_timestampTail; }
void PriorityQueueDataTag::SetFlagData(uint32_t flag) { m_flagData = flag; }
uint32_t PriorityQueueDataTag::GetFlagData(void) const { return m_flagData; }

TypeId PriorityQueueDataTag::GetInstanceTypeId(void) const { return GetTypeId(); }
uint32_t PriorityQueueDataTag::GetSerializedSize(void) const {
    return sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint32_t) +
           sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint32_t);
}
void PriorityQueueDataTag::Serialize(TagBuffer i) const {
    i.WriteU32(m_pathId);
...
void PriorityQueueDataTag::Deserialize(TagBuffer i) {
    m_pathId = i.ReadU32();
...
void PriorityQueueDataTag::Print(std::ostream &os) const {
 ...
 */
PriorityQueueReplyTag::PriorityQueueReplyTag() : Tag() {}
TypeId PriorityQueueReplyTag::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::PriorityQueueReplyTag").SetParent<Tag>().AddConstructor<PriorityQueueReplyTag>();
    return tid;
}
void PriorityQueueReplyTag::SetFlagReply(uint32_t flagReply) { m_flagReply = flagReply; }
uint32_t PriorityQueueReplyTag::GetFlagReply(void) const { return m_flagReply; }
void PriorityQueueReplyTag::SetEpoch(uint32_t epoch) { m_epoch = epoch; }
uint32_t PriorityQueueReplyTag::GetEpoch(void) const { return m_epoch; }
void PriorityQueueReplyTag::SetPhase(uint32_t phase) { m_phase = phase; }
uint32_t PriorityQueueReplyTag::GetPhase(void) const { return m_phase; }
TypeId PriorityQueueReplyTag::GetInstanceTypeId(void) const { return GetTypeId(); }
uint32_t PriorityQueueReplyTag::GetSerializedSize(void) const {
    return sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint32_t);
}
void PriorityQueueReplyTag::Serialize(TagBuffer i) const {
    i.WriteU32(m_flagReply);
    i.WriteU32(m_epoch);
    i.WriteU32(m_phase);
}
void PriorityQueueReplyTag::Deserialize(TagBuffer i) {
    m_flagReply = i.ReadU32();
    m_epoch = i.ReadU32();
    m_phase = i.ReadU32();
}
void PriorityQueueReplyTag::Print(std::ostream &os) const {
 ...
 */
PriorityQueueNotifyTag::PriorityQueueNotifyTag() : Tag() {}
TypeId PriorityQueueNotifyTag::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::PriorityQueueNotifyTag").SetParent<Tag>().AddConstructor<PriorityQueueNotifyTag>();
    return tid;
}
void PriorityQueueNotifyTag::SetPathId(uint32_t pathId) { m_pathId = pathId; }
uint32_t PriorityQueueNotifyTag::GetPathId(void) const { return m_pathId; }
TypeId PriorityQueueNotifyTag::GetInstanceTypeId(void) const { return GetTypeId(); }
uint32_t PriorityQueueNotifyTag::GetSerializedSize(void) const { return sizeof(uint32_t); }
void PriorityQueueNotifyTag::Serialize(TagBuffer i) const { i.WriteU32(m_pathId); }
void PriorityQueueNotifyTag::Deserialize(TagBuffer i) { m_pathId = i.ReadU32(); }
void PriorityQueueNotifyTag::Print(std::ostream &os) const { os << "m_pathId=" << m_pathId; }

/*---------------- PriorityQueueRouting ---------------*/
// debugging to check timing
uint64_t PriorityQueueRouting::debug_time = 0;

// static members for topology information and statistics
uint64_t PriorityQueueRouting::m_nReplyInitSent = 0;
uint64_t PriorityQueueRouting::m_nReplyTailSent = 0;
uint64_t PriorityQueueRouting::m_nTimelyInitReplied = 0;
uint64_t PriorityQueueRouting::m_nTimelyTailReplied = 0;
uint64_t PriorityQueueRouting::m_nNotifySent = 0;
uint64_t PriorityQueueRouting::m_nReRoute = 0;
uint64_t PriorityQueueRouting::m_nOutOfOrderPkts = 0;
uint64_t PriorityQueueRouting::m_nFlushVOQTotal = 0;
uint64_t PriorityQueueRouting::m_nFlushVOQByTail = 0;
std::vector<uint32_t> PriorityQueueRouting::m_historyVOQSize;

// functions
PriorityQueueRouting::PriorityQueueRouting() {
    m_isToR = false;
    ...
    m_priorityqueuePathTable.resize(65536);            // initialize table size
}

PriorityQueueRouting::~PriorityQueueRouting() {}
void PriorityQueueRouting::DoDispose() { m_agingEvent.Cancel(); }
TypeId PriorityQueueRouting::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::PriorityQueueRouting").SetParent<Object>().AddConstructor<PriorityQueueRouting>();
    return tid;
}

uint32_t PriorityQueueRouting::GetOutPortFromPath(const uint32_t &path, const uint32_t &hopCount) {
    return ((uint8_t *)&path)[hopCount];
}

void PriorityQueueRouting::SetOutPortToPath(uint32_t &path, const uint32_t &hopCount,
                                       const uint32_t &outPort) {
    ((uint8_t *)&path)[hopCount] = outPort;
}

uint64_t PriorityQueueRouting::GetFlowKey(uint32_t ip1, uint32_t ip2, uint16_t port1, uint16_t port2) {
    /** IP_ADDRESS: 11.X.X.1 */
...
uint32_t PriorityQueueRouting::DoHash(const uint8_t *key, size_t len, uint32_t seed) {
    uint32_t h = seed;
    if (len > 3) {
...
void PriorityQueueRouting::SendReply(Ptr<Packet> p, CustomHeader &ch, uint32_t flagReply,
                                uint32_t pkt_epoch) {
    qbbHeader seqh;
    seqh.SetSeq(0);
    ...
    PriorityQueueReplyTag priorityqueueReplyTag;
    priorityqueueReplyTag.SetFlagReply(flagReply);
    priorityqueueReplyTag.SetEpoch(pkt_epoch);
    if (flagReply == PriorityQueueReplyTag::INIT) {
        PriorityQueueRouting::m_nReplyInitSent += 1;
        priorityqueueReplyTag.SetPhase(0);
    } else if (flagReply == PriorityQueueReplyTag::TAIL) {
        PriorityQueueRouting::m_nReplyTailSent += 1;
        priorityqueueReplyTag.SetPhase(1);
    } else {
        SLB_LOG("ERROR - Unknown PriorityQueueReplyTag flag:" << flagReply);
        exit(1);
    }

    replyP->AddPacketTag(priorityqueueReplyTag);

    // dummy reply's inDev interface
    replyP->AddPacketTag(FlowIdTag(Settings::PRIORITYQUEUE_CTRL_DUMMY_INDEV));
...
void PriorityQueueRouting::SendNotify(Ptr<Packet> p, CustomHeader &ch, uint32_t pathId) {
    qbbHeader seqh;
    seqh.SetSeq(0);
    seqh.SetPG(ch.udp.pg);
    seqh.SetSport(ch.udp.dport);
    ...
    // attach PriorityQueueNotifyTag
    PriorityQueueNotifyTag priorityqueueNotifyTag;
    priorityqueueNotifyTag.SetPathId(pathId);
    fbP->AddPacketTag(priorityqueueNotifyTag);

    // dummy notify's inDev interface
    fbP->AddPacketTag(FlowIdTag(Settings::PRIORITYQUEUE_CTRL_DUMMY_INDEV));
    ...
    PriorityQueueRouting::m_nNotifySent += 1;
...
void PriorityQueueRouting::RouteInput(Ptr<Packet> p, CustomHeader &ch) {
    // Packet arrival time
    Time now = Simulator::Now();

    // Turn on aging event scheduler if it is not running
    if (!m_agingEvent.IsRunning()) {
        SLB_LOG("PriorityQueue routing restarts aging event scheduling:" << m_switch_id << now);
        m_agingEvent = Simulator::Schedule(m_agingTime, &PriorityQueueRouting::AgingEvent, this);
    }
    ...
    // get priorityqueueDataTag from packet
    PriorityQueueDataTag priorityqueueDataTag;
    bool foundPriorityQueueDataTag = p->PeekPacketTag(priorityqueueDataTag);

    // get SlbControlTag from packet
    PriorityQueueReplyTag priorityqueueReplyTag;
    bool foundPriorityQueueReplyTag = p->PeekPacketTag(priorityqueueReplyTag);

    // get priorityqueueNotifyTag from packet
    PriorityQueueNotifyTag priorityqueueNotifyTag;
    bool foundPriorityQueueNotifyTag = p->PeekPacketTag(priorityqueueNotifyTag);

    if (ch.l3Prot == 0xFD) { /** ACK or PriorityQueue's Control Packets */
        /** NOTE: PriorityQueue uses 0xFD protocol id for its control packets.
         * Quick filter purely (N)ACK packets (not PriorityQueue control packets)
         **/
        if (!foundPriorityQueueReplyTag && !foundPriorityQueueNotifyTag) {  // pure-(N)ACK
        ...
        /** NOTE: PriorityQueue's control packets are forwarded with default flow-ECMP */
        if (!m_isToR) {
            SLB_LOG(PARSE_FIVE_TUPLE(ch) << "PriorityQueue Ctrl Pkts use flow-ECMP at non-ToR switches");
            DoSwitchSendToDev(p, ch);
            return;
        }
    }
    ...
    if (m_isToR) {  // ToR switch
        ...
        if (m_switch_id == srcToRId) {  // SrcToR -
            ...
            assert(!foundPriorityQueueDataTag &&
                   !foundPriorityQueueReplyTag);  // ERROR - dataTag and replyTag
                                             // should not be found at TxToR

            /** PIPELINE: emulating the p4 pipelining */
            priorityqueueTxMeta tx_md;  // SrcToR packet metadata
            tx_md.pkt_flowkey = GetFlowKey(ch.sip, ch.dip, ch.udp.sport, ch.udp.dport);
            auto &txEntry = m_priorityqueueTxTable[tx_md.pkt_flowkey];
             ...
             */
            std::set<uint32_t> pathSet = m_PriorityQueueRoutingTable[dstToRId];  // pathSet to RxToR
            uint32_t initPath =
            ...
            if (m_pathAwareRerouting) {
                ...
                const auto pathEntry1 =
                    m_priorityqueuePathTable[DoHash((uint8_t *)&randPath1, 4, m_switch_id) %
                                        m_priorityqueuePathTable.size()];
                const auto pathEntry2 =
                    m_priorityqueuePathTable[DoHash((uint8_t *)&randPath2, 4, m_switch_id) %
                                        m_priorityqueuePathTable.size()];
                bool goodPath1 = true;
                bool goodPath2 = true;
            ...
            if (tx_md.flagExpired) { /* expired -> update path and use the new path */
                if (tx_md.foundGoodPath) {
                    PriorityQueueRouting::m_nReRoute += (tx_md.newConnection == false ? 1 : 0);
                    txEntry._pathId = tx_md.goodPath;
                    SLB_LOG(PARSE_FIVE_TUPLE(ch)
             ...
             */
            priorityqueueDataTag.SetPathId(tx_md.currPath);
            priorityqueueDataTag.SetHopCount(0);
            priorityqueueDataTag.SetEpoch(tx_md.epoch);
            priorityqueueDataTag.SetPhase(tx_md.phase);
            priorityqueueDataTag.SetTimestampTx(now.GetNanoSeconds());
            priorityqueueDataTag.SetTimestampTail(tx_md.tailTime);
            if (tx_md.flagExpired || tx_md.flagStabilized) { /* ask reply of INIT */
                priorityqueueDataTag.SetFlagData(PriorityQueueDataTag::INIT);
                assert(tx_md.phase == 0);
            } else if (tx_md.flagReplyTimeout) { /* ask reply of TAIL (CLEAR packet) */
                priorityqueueDataTag.SetFlagData(PriorityQueueDataTag::TAIL);
                assert(tx_md.phase == 0);
            } else {
                priorityqueueDataTag.SetFlagData(PriorityQueueDataTag::DATA);
            }
            p->AddPacketTag(priorityqueueDataTag);

            uint32_t outDev =
                GetOutPortFromPath(priorityqueueDataTag.GetPathId(), priorityqueueDataTag.GetHopCount());
            uint32_t qIndex = ch.udp.pg;
            SLB_LOG(PARSE_FIVE_TUPLE(ch)
                    << "\t--> outDev:" << outDev << ",qIndex:" << qIndex
                    << ",pktEpoch:" << tx_md.epoch << ",pktPhase:" << tx_md.phase
                    << ",tailTime:" << tx_md.tailTime << ",pktPath:" << priorityqueueDataTag.GetPathId()
                    << ",flag:" << priorityqueueDataTag.GetFlagData() << " (2:INIT,3:TAIL)");
         ...
         ***************************/
            ...
            if (foundPriorityQueueDataTag) {  // DATA

                /** PIPELINE: emulating the p4 pipelining */
                priorityqueueRxMeta rx_md;
                rx_md.pkt_flowkey = GetFlowKey(ch.sip, ch.dip, ch.udp.sport, ch.udp.dport);
                auto &rxEntry = m_priorityqueueRxTable[rx_md.pkt_flowkey];
                ...
                /**
                 * PARSING: parse packet's priorityqueueDataTag
                 */
                rx_md.pkt_pathId = priorityqueueDataTag.GetPathId();
                rx_md.pkt_epoch = priorityqueueDataTag.GetEpoch();
                rx_md.pkt_phase = priorityqueueDataTag.GetPhase();
                rx_md.pkt_timestamp_Tx = priorityqueueDataTag.GetTimestampTx();
                rx_md.pkt_timestamp_TAIL = priorityqueueDataTag.GetTimestampTail();
                rx_md.pkt_flagData = priorityqueueDataTag.GetFlagData();
                ...
                if (rx_md.resultEpochMatch == 1) { /* new epoch */
                    ...
                    rxEntry._phase = (rx_md.pkt_flagData == PriorityQueueDataTag::TAIL) ? 1 : 0;
                     ...
                     *  Try to increase "priorityqueue_txExpiryTime" if this message appears in many
                     *  times.
                     */
                    ...
                    if (rx_md.pkt_flagData == PriorityQueueDataTag::TAIL) { /* TAIL */
                ...
                if (rx_md.pkt_flagData == PriorityQueueDataTag::TAIL ||
                ...
                if (rx_md.flagPhase0Cache) { /* phase0-timestamp is available */
                    ...
                    /** DEBUGGING: */
                    if (rx_md.pkt_phase == 1 || rx_md.pkt_flagData == PriorityQueueDataTag::TAIL) {
                        if (rx_md.tailTime < rx_md.phase0TxTime.GetNanoSeconds()) {
                            SLB_LOG(PARSE_FIVE_TUPLE(ch)
                                    << "** PRIORITYQUEUE WARNING - Though this pkt has tailTime, the "
                                       "tailTime is before Phase0-TxTime");
                            SLB_LOG(PARSE_FIVE_TUPLE(ch)
                                    << "** tailTime:" << rx_md.tailTime
                                    << ",Phase0TxTime:" << rx_md.phase0TxTime.GetNanoSeconds());
                            std::cout << "** PRIORITYQUEUE WARNING - Though this pkt has tailTime, the "
                                         "tailTime is before Phase0-TxTime"
                                      << std::endl;
                            exit(1);
                        }
                ...
                if (rx_md.pkt_phase == 1) {          /* phase 1 */
                    ...
                    if (rx_md.pkt_flagData == PriorityQueueDataTag::TAIL) { /* TAIL -> Flush VOQ!! */
                        rx_md.timeExpectedToFlush =
                            now.GetNanoSeconds() +
                ...
                if (rx_md.pkt_phase == 0) {    /* phase 0 -> update if currently reordering */
                    if (rxEntry._reordering) { /* reordering */
                        if (rx_md.pkt_flagData == PriorityQueueDataTag::TAIL) {
                            PriorityQueueRouting::m_nFlushVOQByTail += 1; /* debugging */
                        }
                        /* new deadline */
                        auto voq = m_voqMap.find(rx_md.pkt_flowkey);
                    ...
                    if (rx_md.flagOutOfOrder) {   /* out-of-order */
                        ...
                        if (rxEntry._reordering) { /* reordering is on-going */
                            ...
                            PriorityQueueVOQ &voq = m_voqMap[rx_md.pkt_flowkey];
                            ...
                            voq.m_deleteCallback = MakeCallback(&PriorityQueueRouting::DeleteVOQ, this);
                            voq.m_CallbackByVOQFlush =
                                MakeCallback(&PriorityQueueRouting::CallbackByVOQFlush, this);
                            voq.m_switchSendToDevCallback =
                                MakeCallback(&PriorityQueueRouting::DoSwitchSendToDev, this);
                            SLB_LOG(PARSE_FIVE_TUPLE(ch)
                                    << "--> FIRST OoO"
                 ...
                 */
                if (rx_md.pkt_flagData == PriorityQueueDataTag::INIT) {
                    assert(rx_md.pkt_phase == 0);  // sanity check
                    SendReply(p, ch, PriorityQueueReplyTag::INIT, rx_md.pkt_epoch);
                }
                if (rx_md.pkt_flagData == PriorityQueueDataTag::TAIL) {
                    assert(rx_md.pkt_phase == 0);  // sanity check
                    SendReply(p, ch, PriorityQueueReplyTag::TAIL,
            ...
            if (foundPriorityQueueReplyTag) {  // Received REPLY
                priorityqueueTxMeta tx_md;
                tx_md.pkt_flowkey = GetFlowKey(ch.dip, ch.sip, ch.udp.dport, ch.udp.sport);
                tx_md.reply_flag = priorityqueueReplyTag.GetFlagReply();
                tx_md.reply_epoch = priorityqueueReplyTag.GetEpoch();
                tx_md.reply_phase = priorityqueueReplyTag.GetPhase();
                auto &txEntry = m_priorityqueueTxTable[tx_md.pkt_flowkey];
                ...
                if (tx_md.reply_epoch == txEntry._epoch && tx_md.reply_phase == txEntry._phase) {
                    if (tx_md.reply_flag == PriorityQueueReplyTag::INIT) { /* reply of INIT */
                        if (now < txEntry._replyTimer &&
                            ...
                            PriorityQueueRouting::m_nTimelyInitReplied += 1;
                    ...
                    if (tx_md.reply_flag == PriorityQueueReplyTag::TAIL) { /* reply of TAIL */
                        txEntry._stabilized =
                            true;  // out-of-order issue is resolved for this "flowcut"
                        txEntry._replyTimer = CW_MAX_TIME; /* no more timeout */
                        PriorityQueueRouting::m_nTimelyTailReplied += 1;
                        SLB_LOG(PARSE_REVERSE_FIVE_TUPLE(ch)
                                << "[TxToR/GotReplied] Sw(" << m_switch_id << "),PktEpoch:"
                                << tx_md.reply_epoch << ",PktPhase:" << tx_md.reply_phase
                                << ",ReplyFlag:" << tx_md.reply_flag << ",ReplyDL"
            ...
            if (m_pathAwareRerouting) {        // Received NOTIFY
                if (foundPriorityQueueNotifyTag) {  // Received NOTIFY (from ECN)
                    priorityqueueTxMeta tx_md;
                    auto congestedPathId = priorityqueueNotifyTag.GetPathId();
                    auto &pathEntry =
                        m_priorityqueuePathTable[DoHash((uint8_t *)&congestedPathId, 4, m_switch_id) %
                                            m_priorityqueuePathTable.size()];
                    SLB_LOG(PARSE_REVERSE_FIVE_TUPLE(ch)
        ...
        printf(
            ...
            "NOTIFY:%u\n",
            srcToRId, dstToRId, m_switch_id, m_isToR, foundPriorityQueueDataTag, foundPriorityQueueReplyTag,
            foundPriorityQueueNotifyTag);
        assert(false);
    }
     ...
     ******************************/
    if (foundPriorityQueueDataTag) {  // UDP with PathId
        // update hopCount
        uint32_t hopCount = priorityqueueDataTag.GetHopCount() + 1;
        priorityqueueDataTag.SetHopCount(hopCount);
        ...
        SLB_LOG(PARSE_FIVE_TUPLE(ch) << "[NonToR/DATA] Sw(" << m_switch_id << "),"
                                     << "outDev:" << outDev << ",qIndex:" << qIndex
                                     << ",PktEpoch:" << priorityqueueDataTag.GetEpoch()
                                     << ",PktPhase:" << priorityqueueDataTag.GetPhase());
        DoSwitchSend(p, ch, outDev, qIndex);
        return;
    }
...
void PriorityQueueRouting::SetConstants(Time extraReplyDeadline, Time extraVOQFlushTime,
...
void PriorityQueueRouting::SetSwitchInfo(bool isToR, uint32_t switch_id) {
    m_isToR = isToR;
    m_switch_id = switch_id;
}

/** CALLBACK: callback functions  */
void PriorityQueueRouting::DoSwitchSend(Ptr<Packet> p, CustomHeader &ch, uint32_t outDev,
                                   uint32_t qIndex) {
    m_switchSendCallback(p, ch, outDev, qIndex);
}
void PriorityQueueRouting::DoSwitchSendToDev(Ptr<Packet> p, CustomHeader &ch) {
    m_switchSendToDevCallback(p, ch);
}

// used for callback in VOQ
void PriorityQueueRouting::DeleteVOQ(uint64_t flowkey) { m_voqMap.erase(flowkey); }

void PriorityQueueRouting::CallbackByVOQFlush(uint64_t flowkey, uint32_t voqSize) {
    SLB_LOG(
        "#################################################################### VOQ FLush, flowkey: "
    ...
    // update RxEntry
    auto &rxEntry = m_priorityqueueRxTable[flowkey];  // flowcut entry
...
void PriorityQueueRouting::SetSwitchSendCallback(SwitchSendCallback switchSendCallback) {
    m_switchSendCallback = switchSendCallback;
}

void PriorityQueueRouting::SetSwitchSendToDevCallback(SwitchSendToDevCallback switchSendToDevCallback) {
    m_switchSendToDevCallback = switchSendToDevCallback;
}

uint32_t PriorityQueueRouting::GetVolumeVOQ() {
...
void PriorityQueueRouting::AgingEvent() {
    auto now = Simulator::Now();

    auto itr1 = m_priorityqueueTxTable.begin();
    while (itr1 != m_priorityqueueTxTable.end()) {
        if (now - ((itr1->second)._activeTime) > m_agingTime) {
            itr1 = m_priorityqueueTxTable.erase(itr1);
        } else {
            ++itr1;
        }
    }

    auto itr2 = m_priorityqueueRxTable.begin();
    while (itr2 != m_priorityqueueRxTable.end()) {
        if (now - ((itr2->second)._activeTime) > m_agingTime) {
            itr2 = m_priorityqueueRxTable.erase(itr2);
        } else {
            ++itr2;
        }
    }
