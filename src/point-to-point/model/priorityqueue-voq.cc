...
NS_LOG_COMPONENT_DEFINE("PriorityQueueVOQ");

namespace ns3 {

PriorityQueueVOQ::PriorityQueueVOQ() {}
PriorityQueueVOQ::~PriorityQueueVOQ() {}

std::vector<int> PriorityQueueVOQ::m_flushEstErrorhistory; // instantiate static variable

void PriorityQueueVOQ::Set(uint64_t flowkey, uint32_t dip, Time timeToFlush, Time extraVOQFlushTime) {
    m_flowkey = flowkey;
    m_dip = dip;
...
void PriorityQueueVOQ::Enqueue(Ptr<Packet> pkt) { m_FIFO.push(pkt); }

void PriorityQueueVOQ::FlushAllImmediately() {
...
void PriorityQueueVOQ::EnforceFlushAll() {
    SLB_LOG(
        "--> *** Finish this epoch by Timeout Enforcement - PriorityQueueVOQ Size:" << m_FIFO.size());
    PriorityQueueRouting::m_nFlushVOQTotal += 1;  // statistics
 ...
 */
void PriorityQueueVOQ::RescheduleFlush(Time timeToFlush) {
    ...
    m_checkFlushEvent = Simulator::Schedule(timeToFlush, &PriorityQueueVOQ::EnforceFlushAll, this);
}

bool PriorityQueueVOQ::CheckEmpty() { return m_FIFO.empty(); }

uint32_t PriorityQueueVOQ::getQueueSize() { return (uint32_t)m_FIFO.size(); }
