...
#ifndef __PRIORITYQUEUE_ROUTING_H__
#define __PRIORITYQUEUE_ROUTING_H__
...
/************************************************************************************************
 *
*  ---- NOTE: Priorityqueue mechanism on NS-3 simulator. ----
 * #A# The implementation might not be optimal unlike on switching ASIC.
 *
 * #B# We use "tag" instead of "header" class in NS-3. Since PriorityQueue header's extra overhead is
...
struct priorityqueuePathInfo {
    uint32_t _pathId = 0;
    Time _invalidTime = CW_MIN_TIME;
};

struct find_priorityqueuePathInfo {
    uint32_t _pathId;
    find_priorityqueuePathInfo(uint32_t pathId) : _pathId(pathId) {}
    bool operator()(const priorityqueuePathInfo& p) const { return p._pathId == _pathId; }
};
// it = std::find_if( pathVec.begin(), pathVec.end(), priorityqueuePathInfo(pathId));

// follow PISA metadata concept
struct priorityqueueTxMeta {
...
class PriorityQueueDataTag : public Tag {
   public:
    PriorityQueueDataTag();
    ...
    friend std::ostream& operator<<(std::ostream& os, PriorityQueueDataTag const& tag) {
   ...
   private:
    ...
    uint32_t m_flagData = PriorityQueueDataTag::NONE;  // control flag
};

// tag for reply
class PriorityQueueReplyTag : public Tag {
   public:
    PriorityQueueReplyTag();
    ...
    friend std::ostream& operator<<(std::ostream& os, PriorityQueueReplyTag const& tag) {
    ...
    enum PriorityQueueReplyTagFlag {
        NONE = 0, /* Default, unused */
...
class PriorityQueueNotifyTag : public Tag {
   public:
    PriorityQueueNotifyTag();
    void SetPathId(uint32_t pathId);
    uint32_t GetPathId(void) const;
    ...
    friend std::ostream& operator<<(std::ostream& os, PriorityQueueNotifyTag const& tag) {
...
/**
 * @brief PriorityQueue object is created for each ToR Switch
...
class PriorityQueueRouting : public Object {
   ...
   public:
    ...
    const std::unordered_map<uint64_t, PriorityQueueVOQ>& GetVOQMap() { return m_voqMap; }
    ...
    std::map<uint32_t, std::set<uint32_t> >
        m_PriorityQueueRoutingTable;                      // <RxToRId -> set<pathId> > just for reference
    std::map<uint32_t, uint64_t> m_rxToRId2BaseRTT;  // RxToRId -> BaseRTT between TORs(fixed)
    std::vector<priorityqueuePathInfo> m_priorityqueuePathTable;  // pathInfo table
   ...
   private:
    ...
    // local
    std::map<uint64_t, priorityqueueTxState> m_priorityqueueTxTable;  // flowkey -> TxToR's stateful table
    std::map<uint64_t, priorityqueueRxState> m_priorityqueueRxTable;  // flowkey -> RxToR's stateful table

    // VOQ (voq.m_deleteCallback = MakeCallback(&PriorityQueueRouting::deleteVoq, this); )
    std::unordered_map<uint64_t, PriorityQueueVOQ> m_voqMap;  // flowkey -> FIFO Queue
